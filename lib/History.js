...
function History(app, routes, lightRouter) {
  this.app = app;
  this.routes = routes;
  this.lightRouter = lightRouter;
  this.lightstack = [];
...
History.prototype.emptyLightStack = function() {
  this.lightstack = [];
}

History.prototype.lightpush = function(url) {
  this.lightstack.push(window.location.href);
  window.history.pushState(window.history.state, null, url);
  this.lightRouter.run();
}

History.prototype.lightback = function() {
  if (this.lightstack.length) {
    var url = this.lightstack.pop();
  ...
  function onClick(e) {
    ...
    while (el) {
      ...
      if (el.nodeName === 'A' && url) {
        ...
        // this is a special case of a "light" route -- only the url will 
        // change and the user should take care of changing the screen
        if (el.getAttribute && el.getAttribute('data-norender')) {
          e.preventDefault();
          return history.lightpush(url)
        }

        e._tracksLink = el
        history.push(url, render, null, e)
        return
      }
